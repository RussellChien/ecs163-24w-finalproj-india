...
const barLabels = {
  ...
  Industry:
    "Industry (including construction) value added (constant 2015 US$) over time",
...
d3.csv("data/india_dataset.csv").then((rawData) => {
  ...
  let x1 = d3
    ...
    .domain([
      d3.min(rawData, function (d) {
        if (
          d.IndicatorName == "Individuals using the Internet (% of population)"
        ) {
  ...
  let xAxisCall = d3.axisBottom(x1).ticks(7);
  svg
    .append("g")
    .classed("x_axis", true)
  ...
  let yAxisCall = d3.axisLeft(y1).ticks(13);
  svg.append("g").attr("transform", `translate(${50}, ${0})`).call(yAxisCall);

  const filteredData = rawData.filter(
    (d) =>
      d.IndicatorName === "Individuals using the Internet (% of population)" ||
      d.IndicatorCode === "EG.ELC.ACCS.ZS"
  );
  ...
  d3.select("#line_selector").on("change", function (d) {
    ...
    xAxisCall = d3.axisBottom(x1).ticks(7);

    svg.selectAll(".x_axis").transition().duration(5000).call(xAxisCall);

    let updatedFilteredData = rawData.filter(
      (d) =>
        d.IndicatorName === new_option || d.IndicatorCode === "EG.ELC.ACCS.ZS"
    );
    ...
    if (updatedFilteredData[1]["IndicatorName"] == "GDP (current US$)") {
      for (let i = 1993; i <= 2020; i++) {
        if (updatedFilteredData[1][i] != 0) {
          let updated1point = [
            updatedFilteredData[0][i],
            updatedFilteredData[1][i],
          ];
...
d3.csv("data/india_dataset_gdpindicators.csv").then((rawData) => {
  ...
  // x axis
  var xScale = d3.scalePoint().range([0, width]).padding(1).domain(dimensions);
  ...
  var path = svg
    ...
    .on("click", function () {
      ...
      d3.select(this)
        ...
        .style("stroke", isSelected ? "steelblue" : "red")
        .style("stroke-width", isSelected ? "1px" : "3px");
    });
  ...
  svg
    ...
    .attr("class", "brush")
    .each(function (d) {
      d3.select(this).call(brush.move, yScales[d].range());
    })
    .selectAll("rect")
    .attr("x", -8)
    .attr("width", 16);
  ...
  function brushed() {
    ...
    svg
      ...
      .each(function (d) {
        actives.push({
          dimension: d,
          extent: d3.brushSelection(this).map(yScales[d].invert),
        });
      });
...
d3.csv("data/india_dataset_gdpindicators.csv").then((rawData) => {
  let data = rawData.map((d) => ({
    ...
    Industry:
      +d["Industry (including construction) value added (constant 2015 US$)"],
  ...
  function updateChart(selectedOption) {
    ...
    chartTitle.text(barLabels[selectedOption]);
